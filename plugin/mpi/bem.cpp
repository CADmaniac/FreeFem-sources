...
public:
    ...
    virtual Matrix<K> get_local_dense_perm() const = 0;
    ...
    // Build
    virtual void build(VirtualGenerator<K> &mat, const std::vector<htool::R3> &xt, const std::vector<double> &rt, const std::vector<int> &tabt, const std::vector<double> &gt, const std::vector<htool::R3> &xs, const std::vector<double> &rs, const std::vector<int> &tabs, const std::vector<double> &gs) = 0;
    virtual void build_sym(VirtualGenerator<K> &mat, const std::vector<htool::R3> &xt, const std::vector<double> &rt, const std::vector<int> &tabt, const std::vector<double> &gt)  = 0;

    virtual void build_auto(VirtualGenerator<K> &mat, double* xt, double* xs) = 0;
    virtual void build_auto_sym(VirtualGenerator<K> &mat, double* xt)                        = 0;

    virtual ~HMatrixVirt() {};
};


template<class K, template<class> class LR>
class HMatrixImpl : public HMatrixVirt<K> {
private:
    HMatrix<K,LR,RjasanowSteinbach> H;
public:
    ...
    Matrix<K> get_local_dense_perm() const {return H.get_local_dense_perm();}
    ...
    void set_ndofperelt(unsigned int ndofperelt0)     {H.set_ndofperelt(ndofperelt0);}       
    ...
    virtual void build_sym(VirtualGenerator<K> &mat, const std::vector<htool::R3> &xt, const std::vector<double> &rt, const std::vector<int> &tabt, const std::vector<double> &gt) {H.build_sym(mat,xt,tabt);}

    void build_auto(VirtualGenerator<K> &mat, double* xt, double* xs) {H.build_auto(mat,xt,xs);}
    void build_auto_sym(VirtualGenerator<K> &mat, double* xt) {H.build_auto_sym(mat,xt);}                       
};
...
AnyType To(Stack stack,Expression emat,Expression einter,int init)
{
    ...
    if(std::is_same<Type, KNM<K>>::value) {
        Matrix<K> mdense = H.get_local_dense_perm();
        ...
        for (int i=0; i< mdense.nb_rows(); i++)
            for (int j=0; j< mdense.nb_cols(); j++)
                (*M)(i+H.get_local_offset(),j) = mdense(i,j);
...
AnyType SetCompressMat(Stack stack,Expression emat,Expression einter,int init)
{
  ...
  if ( compressor == "" || compressor == "partialACA")
       *Hmat = new HMatrixImpl<K,sympartialACA>(t,t,epsilon,eta);
   else if (compressor == "fullACA")
       *Hmat = new HMatrixImpl<K,fullACA>(t,t,epsilon,eta);
   else if (compressor == "SVD")
       *Hmat = new HMatrixImpl<K,SVD>(t,t,epsilon,eta);
   else {
...
template <class R>
void builHmat(HMatrixVirt<R>** Hmat, VirtualGenerator<R>* generatorP,string compressor,vector<double> &p1,vector<double> &p2,MPI_Comm comm)  {
    std::shared_ptr<Cluster<PCARegularClustering>> t =std::make_shared<Cluster<PCARegularClustering>>();
    ...
    if (compressor=="" || compressor == "partialACA")
        *Hmat = new HMatrixImpl<R,sympartialACA>(t,s,1e-4,10,'N','N',-1,comm);
    else if (compressor == "fullACA")
        *Hmat = new HMatrixImpl<R,fullACA>(t,s,1e-4,10,'N','N',-1,comm);
    else if (compressor == "SVD")
        *Hmat = new HMatrixImpl<R,SVD>(t,s,1e-4,10,'N','N',-1,comm);
    else {
        cerr << "Error: unknown htool compressor \""+compressor+"\"" << endl;
        ffassert(0);
    }
    // TODO set options
    (*Hmat)->build_auto(*generatorP,p2.data(),p1.data());

}

template<class Matrix, class R, typename std::enable_if< std::is_same< Matrix, HMatrixVirt<R>* >::value >::type* = nullptr>
void Assembly(Matrix* A, htool::VirtualGenerator<R>* generator, string compressor,vector<double> &p1,vector<double> &p2,MPI_Comm comm,int dim,bool = false) {
    builHmat<R>(A,generator,compressor,p1,p2,comm);
}
    ...
    *Hmat =0;
    ...
    if (VFBEM==1) {
        ...
        if ( ds.compressor == "" || ds.compressor == "partialACA")
            *Hmat = new HMatrixImpl<R,partialACA>(t,s,ds.epsilon,ds.eta,ds.sym?'S':'N',ds.sym?'U':'N',-1,comm);
        else if (ds.compressor == "fullACA")
            *Hmat = new HMatrixImpl<R,fullACA>(t,s,ds.epsilon,ds.eta,ds.sym?'S':'N',ds.sym?'U':'N',-1,comm);
        else if (ds.compressor == "SVD")
            *Hmat = new HMatrixImpl<R,SVD>(t,s,ds.epsilon,ds.eta,ds.sym?'S':'N',ds.sym?'U':'N',-1,comm);
        else {
        ...
        (*Hmat)->build_auto_sym(*generator,p1.data());
        ...
        ff_POT_Generator<HMatrixVirt<R>*,R,P1,MeshBemtool,SMesh>(Hmat,Pot,dof,mesh,node_output,ds.compressor,p1,p2,comm);
    }
    return Hmat;
